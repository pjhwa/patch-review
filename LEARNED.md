...
### OpenClaw 다중 패치 컨텍스트(세션) 격리 완벽 보장
...
- **원인 분석**: `--session-id` 플래그는 기본 CLI 채널 환경에서는 기존 세션을 덮어씌워버리는 경우가 발생하며, `/new` 명령어로 하드 리셋을 하거나 이전 `.jsonl` 파일을 물리적으로 삭제하지 않으면 컨텍스트(기억)가 분리되지 않고 연결됨.
- **해결 방안 & 교훈**: AI에게 리뷰 대상 페이로드를 넘기기 전, `fs.rmSync`를 활용하여 `~/.openclaw/agents/main/sessions/` 디렉터리의 `.lock` 파일뿐만 아니라 **모든 `.jsonl` 파일까지 즉각적으로 파기(Hard-Reset)**하는 로직을 `queue.ts`에 추가. 이로써 AI가 이전 패치의 분석 결과나 프롬프트를 전혀 기억하지 못하는 '백지' 상태에서 단일 패치만을 검토하도록 강제하여 할루시네이션(Hallucination) 및 컨텍스트 길이 초과(OOM)를 원천 차단함.
