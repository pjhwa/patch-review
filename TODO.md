...
- [ ] 파이프라인 스테이지별 데이터 스키마 정의 (json 파일 포맷 기준)` -> `수집 완료` -> `전처리 완료` -> `AI 분석 완료(담당자 확인)`
- [ ] 대시보드 Overview 화면 개발: 각 제품별 단계별 현황과 패치 개수 실시간 표시
...
- [ ] 수동 트리거 UI 개발: 대시보드 UI에 모든 카테고리를 표시하되, 실제 실행 로직은 `os -> linux` 계열만 작동하도록 구성.

## 5. 담당자 검토 뷰 및 패치 상세 (Review Board & Details)
- [ ] AI 리뷰 결과 테이블 뷰 개발: 담당자가 확인해야 할 최종 패치 목록
- [ ] 세부 패치 모달/페이지 개발: AI가 작성한 치명적인 버그 수정 내역 설명 확인 기능
- [ ] CSV 출력 기능: 최종 검토 대상 패치 목록을 CSV 파일로 다운로드하는 기능 개발

## 6. 테스트 및 검증 (Testing & Validation)
- [ ] 스태프 엔지니어 수준의 코드 리뷰 및 구조적 우아함 검토
- [ ] 전체 파이프라인 수동/자동 전환 통합 테스트
- [ ] UI 랜더링 품질 (모바일 반응형, 애니메이션) 시각적 검증

## 7. 완료 및 문서화 (Documentation)
- [ ] `LEARNED.md` 업데이트 (이슈 및 트러블슈팅 내역)
- [ ] 최신 트렌드의 Fancy한 GitHub README 문서 작성
