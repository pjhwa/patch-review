...
4.  **For Kernel Patches ONLY:** 반드시 내역(History)을 최신에서 과거 순(역순)으로 검토하세요. System Hang, Data Loss 등 최고 심각도를 가진 수정을 포함하는 *첫 번째 버전을 만나면 즉시 그 최신 심각 버전(Latest Critical Version)을 권고 패치(`specific_version`)로 선택하세요. 그보다 오래된 패치나 버그가 없는 더 최신 중간 버전은 권고 패치가 되지 못합니다.
