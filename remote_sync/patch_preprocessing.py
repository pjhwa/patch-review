...
def preprocess_patches():
    ...
    parser.add_argument('--start-days', type=int, default=180, help="Number of days to look back from (earliest)")
    parser.add_argument('--end-days', type=int, default=90, help="Number of days to look back to (latest)")
    args = parser.parse_args()
    
    cutoff_start = datetime.now() - timedelta(days=args.start_days)
    cutoff_end = datetime.now() - timedelta(days=args.end_days)
    print(f"[PREPROCESS] Filter cutoff: Processing patches strictly between {cutoff_start.strftime('%Y-%m-%d')} ({args.start_days} days) and {cutoff_end.strftime('%Y-%m-%d')} ({args.end_days} days ago)")
    ...
    for json_path in json_files:
        try:
            ...
            try:
                ...
                # Check timeframe
                if not (cutoff_start <= pub_dt <= cutoff_end):
                    continue
            except Exception:
    ...
    for key, group in grouped.items():
        ...
        kernel_instruction = ""
        if "kernel" in latest['component']:
            kernel_instruction = " [KERNEL PATCH RULES: 반드시 최신버전부터 역순으로 내역을 살펴, 가장 먼저 심각한 결함(System Hang, Data Loss 등)을 해결하는 버전을 발견 시 그 버전을 권고 버전으로 채택하세요.]"

        latest['review_instructions'] = f"Analyze this '{latest['component']}' patch ({review_note}). Check for System Hang, Data Loss, Boot Fail, or Critical Security. Merge insights from {len(history_context)} previous patches.{kernel_instruction}"
