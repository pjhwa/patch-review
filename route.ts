...
export async function GET(request: Request) {
    ...
    try {
        // 1. Count Collected from filesystem data directories
        const countJsonFiles = (dirName: string) => {
            try {
                const targetDir = path.join(linuxSkillDir, dirName);
                if (fs.existsSync(targetDir)) {
                    return fs.readdirSync(targetDir).filter(f => f.endsWith('.json')).length;
                }
            } catch (e) {
                console.error(`Error counting json files in ${dirName}:`, e);
            }
            return 0;
        };

        counts.redhat.collected = countJsonFiles('redhat/redhat_data');
        counts.oracle.collected = countJsonFiles('oracle/oracle_data');
        counts.ubuntu.collected = countJsonFiles('ubuntu/ubuntu_data');
