...
SECURITY_DIR = os.path.join(CEPH_DATA_DIR, "security")
RELEASES_DIR = os.path.join(CEPH_DATA_DIR, "releases")
...
def parse_patch_date(patch: dict, source_type: str) -> str:
    ...
    if source_type == "release":
        pub = patch.get("published_at") or patch.get("created_at") or ""
    else:
        pub = patch.get("published_at") or patch.get("updated_at") or ""
    if pub:
        return pub[:10]
    return "Unknown"
...
def main():
    ...
    parser.add_argument("--security-dir", type=str, default=SECURITY_DIR, help="Path to security JSON dir")
    parser.add_argument("--releases-dir", type=str, default=RELEASES_DIR, help="Path to releases JSON dir")
    ...
    print(f"   보안 디렉토리: {args.security_dir}")
    print(f"   릴리즈 디렉토리: {args.releases_dir}")
    ...
    print(f"\n🔍 [1/2] 보안 패치 분석 중...")
    security_files = sorted(glob.glob(os.path.join(args.security_dir, "*.json")))
    print(f"   총 {len(security_files)}개 파일 발견")
    ...
    for filepath in security_files:
        ...
        patch_id = patch.get("ghsa_id") or patch.get("cve_id") or os.path.basename(filepath).replace(".json", "")
        summary = patch.get("summary", "")
        description = patch.get("description", "")
        published_at = patch.get("published_at") or patch.get("updated_at") or ""
    ...
    # ---- 2. Release Notes ----
    print(f"\n🔍 [2/2] 릴리즈 패치 분석 중...")
    release_files = sorted(glob.glob(os.path.join(args.releases_dir, "*.json")))
    print(f"   총 {len(release_files)}개 파일 발견")
    ...
    for filepath in release_files:
        ...
        patch_id = patch.get("tag_name") or os.path.basename(filepath).replace(".json", "")
        name = patch.get("name", "")
        body = patch.get("body", "")
        published_at = patch.get("published_at") or patch.get("created_at") or ""
