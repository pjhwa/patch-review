...
const { startDate: TARGET_START_DATE, endDate: TARGET_END_DATE } = dateConfig;
...
// --- WORKERS ---
async function redhatWorker(ctxPage, adv) {
...
async function oracleWorker(ctxPage, adv) {
    await ctxPage.goto(adv.url, { waitUntil: 'domcontentloaded', timeout: 30000 });
    const details = await ctxPage.evaluate(() => {
...
async function ubuntuWorker(ctxPage, adv) {
    await ctxPage.goto(adv.url, { waitUntil: 'domcontentloaded', timeout: 30000 });
    const details = await ctxPage.evaluate(() => {
            ...
            try {
                ...
                const pageAdvisories = await page.evaluate((monthStr) => {
                    ...
                    return links.map(link => {
                        ...
                        // Strict filtering to avoid passing invalid/empty URLs or non-advisories
                        if (!idMatch) {
                            // If it doesn't clearly have an advisory ID, skip it
                            // to avoid navigating to "Thread", "Author", "Date" headers or mailto links.
                            return null;
                        }
...
(async () => {
    ...
    // Workers are now in global scope
    ...
    if (retryMode) {
        ...
        try {
            ...
            console.error('[CONFIG] No collection_failures.json found or invalid format. Nothing to retry!');
            process.exit(0);
        }
